...
class AnalyzeMuESys24(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    super(AnalyzeMuESys24, self).__init__(tree, outfile, **kwargs)
  ...
  def kinematics(self, row):
    if row.mPt < 26 or abs(row.mEta) >= 2.4:
      return False
    if row.ePt < 10 or abs(row.eEta) >= 2.3 or bool(abs(row.eEta) > 1.4442 and abs(row.eEta) < 1.566):
      return False
    return True
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      if Emb and self.is_DY:
        if not bool(row.isZmumu or row.isZee):#not
          continue
