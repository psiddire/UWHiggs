...
for x in mc_samples:
    print x
    files.extend(glob.glob('../results/%s/WorkingAnalyzer/AnalyzeMuE/%s' % (jobid, x)))
...
for i, j in enumerate(jet):
    ...
    outputdir = '../plots/%s/AnalyzeMuE/2016Selections/%s/' % (jobid, s1)
    ...
    if i==0:
        QCD = views.NormalizeView(QCD, 1.0)
        QCD = views.ScaleView(QCD, 10812.3504753)
    if i==1:
        QCD = views.NormalizeView(QCD, 1.0)
        QCD = views.ScaleView(QCD, 6996.38345146)
    if i==2:
        QCD = views.NormalizeView(QCD, 1.0)
        QCD = views.ScaleView(QCD, 2726.92070961)
    if i==3:
        QCD = views.NormalizeView(QCD, 1.0)
        QCD = views.ScaleView(QCD, 974.150542259)
    if i==4:
        QCD = views.NormalizeView(QCD, 1.0)
        QCD = views.ScaleView(QCD, 132.17760849)
    if i==5:
        QCD = views.NormalizeView(QCD, 1.0)
        QCD = views.ScaleView(QCD, 595.251916409)
    if i==6:
        QCD = views.NormalizeView(QCD, 1.0)
        QCD = views.ScaleView(QCD, 424.636155605)
    if i==7:
        QCD = views.NormalizeView(QCD, 1.0)
        QCD = views.ScaleView(QCD, 155.693496108)
    if i==8:
        QCD = views.NormalizeView(QCD, 1.0)
        QCD = views.ScaleView(QCD, 14.0129942894)
    ...
    new_mc_samples = []
    new_mc_samples.extend(['DY', 'TT', 'ST', 'W', 'EWKDiboson', 'EWK', 'SMH', 'QCD'])
