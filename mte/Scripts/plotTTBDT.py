...
for x in Lists.mc_samples:
    print x
    Lists.files.extend(glob.glob('../results/%s/AnalyzeMuETTBDT/%s' % (jobid, x)))
...
for j in Lists.jetc:
    ...
    outputdir = 'plots/%s/AnalyzeMuETTBDT/2016SelectionsBDT/%s/' % (jobid, s1)
    ...
    Wtotal = views.SumView( *[ plotter.get_view(regex) for regex in filter(lambda x : x.startswith('W') or x.startswith('W1') or x.startswith('W2') or x.startswith('W3') or x.startswith('W4') or x.startswith('WG'), Lists.mc_samples)])
