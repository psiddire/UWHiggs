...
for x in Lists.mc_samples:
    Lists.files.extend(glob.glob('../results/%s/AnalyzeMuESys24/%s.root' % (jobid, x)))
    Lists.lumifiles.extend(glob.glob('../inputs/%s/%s.lumicalc.sum' % (jobid, x)))

outputdir = 'Shapes/'
plotter = Plotter(Lists.files, Lists.lumifiles, outputdir)
f = ROOT.TFile( outputdir+'shapesMuEDY24.root', 'RECREATE')

v = [
views.SumView( *[ plotter.get_view(regex) for regex in filter(lambda x : x.startswith('DY'), Lists.mc_samples )]),
views.SumView( *[ plotter.get_view(regex) for regex in filter(lambda x : x.startswith('ZTauTau'), Lists.mc_samples )]),
...
for k, di in enumerate(Lists.dirs):
    ...
    data = Lists.positivize(DataTotal.Get('TightOS'+di+'/m_e_CollinearMass'))
    ...
    for i, sam in enumerate(Lists.sampdy):
        ...
        if sam in Lists.norecsampdy:
            for j, jSys in enumerate(Lists.jesSys):
                dy = Lists.positivize(DY.Get(jSys+'m_e_CollinearMass'))
