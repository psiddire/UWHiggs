...
for j in Lists.jetc:
    ...
    outputdir = 'plots/%s/AnalyzeMuEZTT/2016SelectionsEmbedPhi/%s/' % (jobid, s1)
    ...
    DY = views.StyleView(DYall, **Lists.Lists.remove_name_entry(data_styles['DY*']))
    ...
    embed = views.StyleView(embedall, **Lists.Lists.remove_name_entry(data_styles['DYTT*']))
    ...
    EWK = views.StyleView(EWKall, **Lists.Lists.remove_name_entry(data_styles['EWK*']))                                 
    ...
    SMH = views.StyleView(SMHall, **Lists.Lists.remove_name_entry(data_styles['*HToTauTau*'])) 
    ...
    TT = views.StyleView(TTall, **Lists.Lists.remove_name_entry(data_styles['TT*']))
    ...
    Diboson = views.StyleView(Dibosonall, **Lists.Lists.remove_name_entry(data_styles['WZ*']))
    ...
    QCD = views.StyleView(views.SumView(QCD, W), **Lists.Lists.remove_name_entry(data_styles['QCD*']))
    QCD = views.TitleView(QCD, "QCD")

    vbfHMT = views.StyleView(views.SumView( *[ plotter.get_view(regex) for regex in filter(lambda x : 'VBF_LFV_HToMuTau' in x , Lists.mc_samples)]), **Lists.Lists.remove_name_entry(data_styles['VBF_LFV*']))
    ggHMT = views.StyleView(views.SumView( *[ plotter.get_view(regex) for regex in filter(lambda x : 'GluGlu_LFV_HToMuTau' in x , Lists.mc_samples)]), **Lists.Lists.remove_name_entry(data_styles['GluGlu_LFV*']))
