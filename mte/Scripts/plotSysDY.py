...
for x in Lists.mc_samples:
    print x
    Lists.files.extend(glob.glob('../results/%s/AnalyzeMuESys24/%s' % (jobid, x)))
...
for j in Lists.jetc:
    ...
    outputdir = 'plots/%s/AnalyzeMuE/2016SelectionsSys24/%s/' % (jobid, s1)
    if not os.path.exists(outputdir):
        os.makedirs(outputdir)

    plotter = Plotter(Lists.files, Lists.lumifiles, outputdir)

    DYtotal = views.SumView( *[ plotter.get_view(regex) for regex in filter(lambda x : x.startswith('DY') or x.startswith('ZTauTau'), Lists.mc_samples)])
