...
f = ROOT.TFile( 'Shapes/shapesMuEBDTQCD.root', 'RECREATE')
...
for k, di in enumerate(Lists.dirs):
    ...
    d.cd()
    if di=='0Jet':
        binning = array.array('d', [-0.6, -0.55, -0.5, -0.45, -0.4, -0.35, -0.3, -0.25, -0.2, -0.15, -0.1, -0.05, 0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3])
    elif di=='1Jet':
        binning = array.array('d', [-0.65, -0.55, -0.5, -0.45, -0.4, -0.35, -0.3, -0.25, -0.2, -0.15, -0.1, -0.05, 0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3])
    elif di=='2Jet':
        binning = array.array('d', [-0.6, -0.55, -0.5, -0.45, -0.4, -0.35, -0.3, -0.25, -0.2, -0.15, -0.1, -0.05, 0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3])
    else:
        binning = array.array('d', [-0.6, -0.55, -0.5, -0.45, -0.4, -0.35, -0.3, -0.25, -0.2, -0.15, -0.1, -0.05, 0.0, 0.05, 0.1, 0.15, 0.3])
    ...
    #QCD
    qcdSys = []
    data_view = views.SumView( *[ plotter.get_view(regex) for regex in filter(lambda x : x.startswith('Obs'), mc_samples)])
    mc_view = views.SumView( *[ plotter.get_view(regex) for regex in filter(lambda x : x.startswith('Bac'), mc_samples)])
    ...
    for i, qSys in enumerate(Lists.qcdSys):
        qcd = positivize(QCD.Get(qSys+'bdtDiscriminator'))
    ...
    for i, sam in enumerate(Lists.samp):
        ...
        for j, mSys in enumerate(Lists.mcSys):
            dy = positivize(DY.Get(mSys+'bdtDiscriminator'))
        ...
        if sam=='WG' or sam=='vH_htt' or sam=='TT' or sam=='T' or sam=='Diboson':
            for j, jSys in enumerate(Lists.jesSys):
                dy = positivize(DY.Get(jSys+'bdtDiscriminator'))
