...
outputdir = 'plots/%s/AnalyzeMuEQCD/2016Selections/%s/' % (jobid, s[0])
...
for h in Lists.histoname:
    plotter.plot_mc_vs_data('', ['VBF_LFV_HToMuTau_M125*', 'GluGlu_LFV_HToMuTau_M125*'], h[0], 1, xaxis = h[1], leftside=False, xrange=None, preprocess=None, show_ratio=True, ratio_range=1.5, sort=True, blind_region=True, control=s1, jets='', year='2016', channel='etaumu')
    plotter.save(h[0])
