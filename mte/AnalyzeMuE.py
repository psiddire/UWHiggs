...
class AnalyzeMuE(MegaBase, MuEBase):
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      if row.evt==8255039:
        print "Pass 1: ", row.evt, round(myMuon.Pt(), 3), round(myMuon.Eta(), 3), round(myMuon.Phi(), 3), round(row.mRelPFIsoDBDefaultR04, 3), round(myEle.Pt(), 3), round(myEle.Eta(), 3), round(myEle.Phi(), 3), round(row.eRelPFIsoRho, 3), round(myMET.Pt(), 3), round(myMET.Phi(), 3), round(row.j1pt, 3), round(row.j1eta, 3), round(row.j1phi, 3), round(row.j2pt, 3), round(row.j2eta, 3), round(row.j2phi, 3), round(row.vbfMass, 3), row.jetVeto30, row.bjetDeepCSVVeto20Medium_2018_DR0p5, row.rho
