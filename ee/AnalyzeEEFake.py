...
if bool('DYJetsToLL_M-50' in target):
  ...
  pucorrector = mcCorrections.make_puCorrector('singlee', None, 'W')
elif bool('W1JetsToLNu' in target):
  pucorrector = mcCorrections.make_puCorrector('singlee', None, 'W1')
elif bool('W2JetsToLNu' in target):
  pucorrector = mcCorrections.make_puCorrector('singlee', None, 'W2')
elif bool('W3JetsToLNu' in target):
  pucorrector = mcCorrections.make_puCorrector('singlee', None, 'W3')
elif bool('W4JetsToLNu' in target):
  pucorrector = mcCorrections.make_puCorrector('singlee', None, 'W4')
...
class AnalyzeEEFake(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    self.is_recoilC = bool(self.is_DYlow or self.is_DY or self.is_EWK or self.is_W)
    ...
    super(AnalyzeEEFake, self).__init__(tree, outfile, **kwargs)
  ...
  def obj1_id(self, row):
    return bool(row['e1MVANoisoWP90'])
  ...
  def obj2_id(self, row):
    return bool(row['e2MVANoisoWP90'])
  ...
  def obj2_loose(self, row):
    return bool(row['e2RelPFIsoRho'] < 0.5)
  ...
  def copyrow(self, row):
    ...
    subtemp["e1MVANoisoWP90"] = row.e1MVANoisoWP90
    subtemp["e2MVANoisoWP90"] = row.e2MVANoisoWP90
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      if not self.obj1_iso(nrow):
        continue

      if not self.obj2_loose(nrow):
        continue
      ...
      if not self.is_data:
        ...
        e1id = wmc.function("e_id90_kit_ratio").getVal()
        ...
        e2id = wmc.function("e_id90_kit_ratio").getVal()
      ...
      if transverseMass(newrow['e1Pt'], newrow['e1Eta'], newrow['e1Phi'], newrow['e1Mass'], newrow['type1_pfMetEt'], newrow['type1_pfMetPhi']) < 60:
        continue

      mye = ROOT.TLorentzVector()
      mye.SetPtEtaPhiM(newrow['e1Pt'], newrow['e1Eta'], newrow['e1Phi'], newrow['e1Mass'])

      mymet = ROOT.TLorentzVector()
      mymet.SetPtEtaPhiM(newrow['type1_pfMetEt'], 0, newrow['type1_pfMetPhi'], 0)

      myw = ROOT.TLorentzVector()
      myw = mye + mymet

      wpt = myw.Pt()

      if newrow['e2Pt']/wpt < 0.7 or newrow['e2Pt']/wpt > 1.3:
        continue

      if deltaPhi(myw.Phi(), newrow['e2Phi']) < 2.4:
        continue
     
      self.fill_histos(newrow, weight, 'eleloose')

      if not self.obj2_loose(newrow):
        continue
      self.fill_histos(newrow, weight, 'eletight')
