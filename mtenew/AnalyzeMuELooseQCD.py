...
class AnalyzeMuELooseQCD(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    super(AnalyzeMuELooseQCD, self).__init__(tree, outfile, **kwargs)
  ...
  def obj1_id(self, row):
    return (bool(row.mPFIDLoose) and bool(abs(row.mPVDZ) < 0.2) and bool(abs(row.mPVDXY) < 0.045))
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      if self.is_mc:
        ...
        mID = self.muonLooseID(myMuon.Pt(), abs(myMuon.Eta()))
        mIso = self.muonLooseIsoLooseID(myMuon.Pt(), abs(myMuon.Eta()))
