...
from bTagSF import PromoteDemote, PromoteDemoteSyst
...
class AnalyzeMuESys(MegaBase):
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      nbtagup = row.bjetDeepCSVVeto20Medium_2017_DR0p5
      nbtagdown = row.bjetDeepCSVVeto20Medium_2017_DR0p5
      bpt_1 = row.jb1pt
      bflavor_1 = row.jb1hadronflavor
      beta_1 = row.jb1eta
      nbtags = [nbtag, nbtagup, nbtagdown]
      if (not self.is_data and not self.is_embed and nbtag > 0):
        nbtags = PromoteDemoteSyst(self.h_btag_eff_b, self.h_btag_eff_c, self.h_btag_eff_oth, nbtag, bpt_1, bflavor_1, beta_1)
