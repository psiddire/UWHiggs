...
class AnalyzeMuETTbar(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    super(AnalyzeMuETTbar, self).__init__(tree, outfile, **kwargs)
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      if (self.is_mc):
        btagweight1 = bTagEventWeight(nbtag, row.jb1pt, row.jb1hadronflavor, row.jb2pt, row.jb2hadronflavor, 1, 0, 1) 
        btagweight2 = bTagEventWeight(nbtag, row.jb1pt, row.jb1hadronflavor, row.jb2pt, row.jb2hadronflavor, 1, 0, 2)
        weight = weight * (btagweight1 + btagweight2)
      if (self.is_data and nbtag < 1):
        weight = 0 
