...
for x in mc_samples:
    files.extend(glob.glob('results/%s/AnalyzeMuESysFake/%s.root' % (jobid, x)))
    lumifiles.extend(glob.glob('inputs/%s/%s.lumicalc.sum' % (jobid, x)))

outputdir = 'plots/%s/AnalyzeMuESysFake/' % (jobid)
if not os.path.exists(outputdir):
    os.makedirs(outputdir)

plotter = Plotter(files, lumifiles, outputdir)

f = ROOT.TFile( 'shapesMuEFake.root', 'RECREATE')
...
for di in dirs:
    ...
    fakesEle = views.SubdirectoryView(QCD, 'EleLooseOS'+di)
    qcd = fakesEle.Get("m_e_CollinearMass")
    qcd = qcd.Rebin(len(binning)-1, "Fakes", binning)
    efakesup = fakesEle.Get("EleFakeUp/m_e_CollinearMass")
    efakesdown = fakesEle.Get("EleFakeDown/m_e_CollinearMass")
    efakesup = efakesup.Rebin(len(binning)-1, "Fakes_EleFakesUp", binning) 
    efakesdown = efakesdown.Rebin(len(binning)-1, "Fakes_EleFakesDown", binning)
    ...
    for i in range(13):
        ...
        if b[i]=='GluGlu125' or b[i]=='VBF125':
            ...
            DYefakes = views.SubdirectoryView(DYtotal, 'EleLooseOS'+di)
            DY = SubtractionView(DYall, DYefakes, restrict_positive=True)
            dy = DY.Get("m_e_CollinearMass")
        ...
        dyeesup = dyeesup.Rebin(len(binning)-1, b[i]+"_CMS_EES_13TeVUp", binning)
        dyeesdown = DY.Get("eesDown/m_e_CollinearMass")
        dyeesdown = dyeesdown.Rebin(len(binning)-1, b[i]+"_CMS_EES_13TeVDown", binning)
        ...
        if not b[i]=='GluGlu125' and not b[i]=='VBF125':
            dyall = DYall.Get("m_e_CollinearMass")
            dyefakesup = DYefakes.Get("EleFakeUp/m_e_CollinearMass")
            dyefakesdown = DYefakes.Get("EleFakeDown/m_e_CollinearMass")
            dy1 = dyall.Clone()
            dy1e = dyefakesup.Clone()
            dy1.add(dy1e, -1)
            dy1 = positivize(dy1)
            dy1 = dy1.Rebin(len(binning)-1, b[i]+"_CMS_EleFakeRate_13TeVUp", binning)
            dy2 = dyall.Clone()
            dy2e = dyefakesdown.Clone()
            dy2.add(dy2e, -1)
            dy2 = positivize(dy2)
            dy2 = dy2.Rebin(len(binning)-1, b[i]+"_CMS_EleFakeRate_13TeVDown", binning)
            dyall.Delete()
            dy1e.Delete()
            dy2e.Delete()
            dyefakesup.Delete()
