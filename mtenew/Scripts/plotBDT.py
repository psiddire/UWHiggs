...
for x in mc_samples:
    print x
    files.extend(glob.glob('../results/%s/AnalyzeMuESysBDT/%s' % (jobid, x)))
...
for j in jet:
    ...
    outputdir = '../plots/%s/AnalyzeMuEBDT/2016Selections/%s/' % (jobid, s1)
