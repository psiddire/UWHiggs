...
mc_samples = ['DYJetsToLL_M-50*', 'DYJetsToLL_M-10to50*', 'GluGlu_LFV*', 'VBF_LFV*', 'GluGluHToTauTau*', 'VBFHToTauTau*', 'WminusHToTauTau*', 'WplusHToTauTau*', 'ttHToTauTau*', 'ZHToTauTau*', 'TTTo2L2Nu*', 'TTToSemiLeptonic*', 'TTToHadronic*', 'ST_tW_antitop*', 'ST_tW_top*', 'ST_t-channel_antitop*', 'ST_t-channel_top*', 'QCD*', 'WZ*', 'WW*', 'ZZ*', 'EWKWMinus2Jets*', 'EWKWPlus2Jets*', 'EWKZ2Jets_ZToLL*', 'EWKZ2Jets_ZToNuNu*', 'WJetsToLNu*', 'W1JetsToLNu*', 'W2JetsToLNu*', 'W3JetsToLNu*', 'W4JetsToLNu*', 'WGToLNuG*', 'MC*', 'embed*', 'data*']#'DY1JetsToLL*', 'DY2JetsToLL*', 'DY3JetsToLL*', 'DY4JetsToLL*', 
...
for x in mc_samples:
    print x
    files.extend(glob.glob('results/%s/AnalyzeMuEZTTHTT/%s' % (jobid, x)))
...
for j in jet:
    ...
    outputdir = 'plots/%s/AnalyzeMuEZTT/EmbedHTTNewNtupleCor/%s/' % (jobid, s1)
    if not os.path.exists(outputdir):
        os.makedirs(outputdir)

    plotter = Plotter(files, lumifiles, outputdir)

    DYtotal = views.SumView( *[ plotter.get_view(regex) for regex in filter(lambda x :  x.startswith('DYJetsToLL_M-50') , mc_samples)])#x.startswith('DY1') or x.startswith('DY2') or x.startswith('DY3') or x.startswith('DY4') or 
    ...
    histoname = [("mPt", "Muon  Pt (GeV)", 1),("ePt", "Electron Pt (Gev)", 1),("mEta", "Muon Eta", 1),("eEta", "Electron Eta", 1),("mPhi", "Muon Phi", 2),("ePhi", "Electron Phi", 1),("type1_pfMetEt", "Type1 MET Et", 1),("type1_pfMetPhi", "Type1 MET Phi", 1),("m_e_Mass", "Muon + Electron Mass (GeV)", 1),("m_e_CollMass", "Muon + Electron Collinear Mass (GeV)", 1),("numOfJets", "Number of Jets", 1),("numOfVtx", "Number of Vertices", 1),("dEtaMuE", "Delta Eta Mu E", 1),("dPhiEMET", "Delta Phi E MET", 1),("dPhiMuMET", "Delta Phi Mu MET", 1),("dPhiMuE", "Delta Phi Mu E", 1),("MTEMET", "Electron MET Transverse Mass (GeV)", 1),("MTMuMET", "Mu MET Transverse Mass (GeV)", 1)]
    ...
    for fn in foldername:
        ...
        for n,h in enumerate(histoname):
            plotter.plot_mc_vs_data(fn, ['VBF_LFV_HToMuTau_M125*', 'GluGlu_LFV_HToMuTau_M125*'], h[0], 1, xaxis = h[1], leftside=False, xrange=None, preprocess=None, show_ratio=True, ratio_range=1.5, sort=True, blind_region=False, control=s1, jets=j, channel='e')
            plotter.save(h[0])
