...
for x in mc_samples:
    print x
    files.extend(glob.glob('results/%s/AnalyzeMMMMerged/%s' % (jobid, x)))
    lumifiles.extend(glob.glob('inputs/%s/%s.lumicalc.sum' % (jobid, x)))

#dirs = ['initial', 'muonloose', 'muontight']
dirs = ['muonloose', 'muontight']

for d in dirs:

    outputdir = 'plots/%s/AnalyzeMMMMerged/%s/' % (jobid, d)
    ...
    histoname = [("mPt", "Muon p_{T} (GeV)", 1), ("mEta", "Muon #eta", 1)]
    ...
    for fn in foldername:
        ...
        for n,h in enumerate(histoname):
            plotter.plot_mc_vs_data(fn, [], h[0], 1, xaxis = h[1], leftside=False, xrange=None, preprocess=None, show_ratio=True, ratio_range=1.5, sort=True, blind_region=True, control=d, jets='', year='2016', channel='mumumu')
            plotter.save(h[0])
