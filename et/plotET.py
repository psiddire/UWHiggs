...
for j in jet:
    ...
    outputdir = 'plots/%s/AnalyzeETau/Mar7/%s/' % (jobid, s1)
    ...
    EWK = views.StyleView(SubtractionView(EWKall, EWKtfakes, restrict_positive=True), **remove_name_entry(data_styles['W*Jets*']))
    #EWK = EWKall
...
#    plotter.views['embed']={'view' : embed }
    ...
    for fn in foldername:
        ...
        for n,h in enumerate(histoname):
            ...
            plotter.plot_mc_vs_data(fn, ['VBF_LFV_HToETau_M125*', 'GluGlu_LFV_HToETau_M125*'], h[0], 1, xaxis = h[1], leftside=False, xrange=None, preprocess=None, show_ratio=True, ratio_range=1.5, sort=True, blind_region=True, control=s1, jets=j)
            plotter.save(h[0])
