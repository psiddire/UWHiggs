...
class ETauBase():
  ...
  def corrFact(self, row, myEle, myTau):
    ...
    if self.is_mc:
      ...
      weight = weight*row.GenWeight*pucorrector[''](row.nTruePU)*tEff*eID*row.prefiring_weight
    ...
    if self.is_embed:
      ...
      weight = weight*row.GenWeight*dm*esel*tsel*trgsel*e_trg_sf*e_idiso_sf*e_trk_sf*self.EmbedEta(myEle.Eta(), njets, mjj)*self.EmbedPhi(myEle.Phi(), njets, mjj)*self.EmbedPt(myEle.Pt(), njets, mjj)
      # Tau Identification
      if self.obj2_tight(row):
