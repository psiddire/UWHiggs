...
class AnalyzeETauZTTBDT(MegaBase, ETauBase):
  ...
  def fill_histos_gen(self, row, myEle, myMET, myTau, weight, name=''):
    ...
    if self.oppositesign(row):
      self.fill_histos(myEle, myMET, myTau, weight, name)
      if njets==0:
        self.fill_histos(myEle, myMET, myTau, weight, name+'0Jet')
      elif njets==1:
        self.fill_histos(myEle, myMET, myTau, weight, name+'1Jet')
      elif njets==2 and mjj < 500:
        self.fill_histos(myEle, myMET, myTau, weight, name+'2Jet')
      elif njets==2 and mjj > 500:
        self.fill_histos(myEle, myMET, myTau, weight, name+'2JetVBF')
