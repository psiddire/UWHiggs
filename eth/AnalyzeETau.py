  ...
  def fill_histos_gen(self, row, myEle, myMET, myTau, weight, name):
    ...
    if self.oppositesign(row):
      if self.transverseMass(myEle, myMET) > 60 and self.transverseMass(myTau, myMET) > 80:
        self.fill_histos(row, myEle, myMET, myTau, weight, name[0])
        if njets==0:
          self.fill_histos(row, myEle, myMET, myTau, weight, name[0]+'0Jet')
        elif njets==1:
          self.fill_histos(row, myEle, myMET, myTau, weight, name[0]+'1Jet')
        elif njets==2 and mjj < 500:
          self.fill_histos(row, myEle, myMET, myTau, weight, name[0]+'2Jet')
        elif njets==2 and mjj > 500:
    ...
    if not self.oppositesign(row):
      ...
      if njets==0:
        self.fill_histos(row, myEle, myMET, myTau, weight, name[2]+'0Jet')
      elif njets==1:
        self.fill_histos(row, myEle, myMET, myTau, weight, name[2]+'1Jet')
      elif njets==2 and mjj < 500:
        self.fill_histos(row, myEle, myMET, myTau, weight, name[2]+'2Jet')
      elif njets==2 and mjj > 500:
        self.fill_histos(row, myEle, myMET, myTau, weight, name[2]+'2JetVBF')
