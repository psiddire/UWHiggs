...
for x in mc_samples:
    print x
    files.extend(glob.glob('../results/%s/AnalyzeETauSysBDT/%s' % (jobid, x)))
...
for j in jet:
    ...
    outputdir = 'plots/%s/AnalyzeETau/2017SelectionsEmbedSysBDTNewFakeDYRebin/%s/' % (jobid, s1)
    ...
    for fn in foldername:
        ...
        for n,h in enumerate(histoname):
            ...
            plotter.plot_mc_vs_data(fn, ['VBF_LFV_HToETau_M125*', 'GluGlu_LFV_HToETau_M125*'], h[0], 5, xaxis = h[1], leftside=False, xrange=[-0.7,0.4], preprocess=None, show_ratio=True, ratio_range=1.5, sort=True, blind_region=True, control=s1, jets=j, year='2016', channel='etauh')
            plotter.save(h[0])
