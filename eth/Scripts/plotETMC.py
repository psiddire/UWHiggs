...
for x in mc_samples:
    print x
    files.extend(glob.glob('../results/%s/AnalyzeETau/%s' % (jobid, x)))
...
for j in jet:
    ...
    outputdir = 'plots/%s/AnalyzeETau/2016Selections/%s/' % (jobid, s1)
    ...
    DYlowefakes = views.SubdirectoryView(DYlowtotal, s3)
    DYlowetfakes = views.SubdirectoryView(DYlowtotal, s4)
    DYlowet = views.SumView(DYlowtfakes, DYlowefakes)
    DYlowfakes = SubtractionView(DYlowet, DYlowetfakes, restrict_positive=True)
    ...
    STefakes = views.SubdirectoryView(STtotal, s3)
    STetfakes = views.SubdirectoryView(STtotal, s4)
    STet = views.SumView(STtfakes, STefakes)
    STfakes = SubtractionView(STet, STetfakes, restrict_positive=True)
    ...
    histoname = [("ePt", "Electron Pt (GeV)", 1),("tPt", "Tau Pt (GeV)", 1),("eEta", "Electron Eta", 1),("tEta", "Tau Eta", 1),("ePhi", "Electron Phi", 1),("tPhi", "Tau Phi", 1),("type1_pfMetEt", "Type1 MET Et", 1),("type1_pfMetPhi", "Type1 MET Phi", 1),("e_t_Mass", "Electron + Tau Mass (GeV)", 1),("e_t_CollinearMass", "Electron + Tau Collinear Mass (GeV)", 1),("numOfJets", "Number of Jets", 1),("numOfVtx", "Number of Vertices", 1),("dEtaETau", "Delta Eta E Tau", 1),("dPhiEMET", "Delta Phi E MET", 1),("dPhiTauMET", "Delta Phi Tau MET", 1),("dPhiETau", "Delta Phi E Tau", 1),("MTEMET", "Electron MET Transverse Mass (GeV)", 1),("MTTauMET", "Tau MET Transverse Mass (GeV)", 1)]
    ...
    for fn in foldername:
        ...
        for n,h in enumerate(histoname):
            ...
            plotter.plot_mc_vs_data(fn, ['VBF_LFV_HToETau_M125*', 'GluGlu_LFV_HToETau_M125*'], h[0], 1, xaxis = h[1], leftside=False, xrange=None, preprocess=None, show_ratio=True, ratio_range=1.5, sort=True, blind_region=True, control=s1, jets=j, channel='etauh')
            plotter.save(h[0])
