...
for x in Lists.mc_samples:
    print x
    Lists.files.extend(glob.glob('../results/%s/AnalyzeETauSysDeep/%s' % (jobid, x)))
...
for j in Lists.jet:
    ...
    outputdir = 'plots/%s/AnalyzeETauSys/2016SelectionsEmbedDeep/%s/' % (jobid, s1)
    ...
    for fn in foldername:
        ...
        for n,h in enumerate(Lists.colhisto):
            #plotter.pad.SetLogy(True)
            plotter.plot_mc_vs_data(fn, ['VBF_LFV_HToETau_M125*', 'GluGlu_LFV_HToETau_M125*'], h[0], 5, xaxis = h[1], leftside=False, xrange=None, preprocess=None, show_ratio=True, ratio_range=1.5, sort=True, blind_region=True, control=s1, jets=j, year='2016', channel='etauh')
            plotter.save(h[0])
