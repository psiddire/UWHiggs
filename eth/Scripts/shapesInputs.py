...
for x in Lists.mc_samples:
    Lists.files.extend(glob.glob('../results/%s/AnalyzeETau/%s.root' % (jobid, x)))
...
mfakes = QCD.Get('EleLooseOS'+fol+'/'+var)
mtfakes = QCD.Get('EleLooseTauLooseOS'+fol+'/'+var)
qcd = tfakes.Clone()
qcd.Add(mfakes)
qcd.Add(mtfakes, -1)
...
mfakes_mc = MC.Get('EleLooseOS'+fol+'/'+var)
mtfakes_mc = MC.Get('EleLooseTauLooseOS'+fol+'/'+var)
mc = tfakes_mc.Clone()
mc.Add(mfakes_mc)
mc.Add(mtfakes_mc, -1)
