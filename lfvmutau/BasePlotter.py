...
class BasePlotter(Plotter):
    def __init__ (self, files, lumifiles, outputdir, blind_region=None, forceLumi=-1, use_embedded=False): 
        ...
        files     = glob.glob('results/%s/NewAnalyzeMuTau/*.root' % jobid)
    ...
    def simpleplot_mc(self, folder, signal, variable, rebin=1, xaxis='',
                      leftside=False, xrange=None, preprocess=None, sort=True,forceLumi=-1):
...
#                signal_view=self.get_wild_dir(
#                    self.rebin_view(signal_view,rebin),folder)
#                sign[0]=signal_view.Get(variable)
#                signalview.append(sign[0])
#                sign[0].Draw("SAME")
#                if sign[0].GetBinContent(signal.GetMaximumBin()) > mymax:
#                    mymax = sign[0].GetBinContent(sign[0].GetMaximumBin())
#                    
#                self.keep.append(sign[0])
#        for sig in sign[1]:
#                signal_view=self.get_view(sig)
#                if preprocess:
#                    signal_view=preprocess(signal_view)
#                signal_view=self.get_wild_dir(
#                    self.rebin_view(signal_view,rebin),folder)
#                sign[1]=signal_view.Get(variable)
#                signalview.append(sign[1])
#                sign[1].Draw("SAME")
#                if sign[1].GetBinContent(signal.GetMaximumBin()) > mymax:
#                    mymax = sign[1].GetBinContent(sign[1].GetMaximumBin())
#                    
#                self.keep.append(sign[1])
#
