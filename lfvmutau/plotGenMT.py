...
files=[]
lumifiles=[]
channel = ['initial', 'oppSign', 'passTrigger', 'passKinematics', 'passObj1id', 'passObj1iso', 'passObj2id', 'passObj2iso', 'passVetoes', 'passallselections']
for x in mc_samples:
    print x
...
period = '13TeV'
sqrts = 13
outputdir = 'plots/%s/NewAnalyzeMuTau/9AugNew/' % (jobid)
...
#x.startswith('DYJetsToLL') or x.startswith('DY1JetsToLL') or x.startswith('DY2JetsToLL') or x.startswith('DY3JetsToLL') or x.startswith('DY4JetsToLL')
...
#x.startswith('W1JetsToLNu') or x.startswith('W2JetsToLNu') or x.startswith('W3JetsToLNu') or x.startswith('W4JetsToLNu') or x.startswith('WJetsToLNu')
...
histoname = [('m_t_Mass','Muon + Tau Mass',1),('m_t_CollinearMass','Muon + Tau Collinear Mass',1),("mPt", "Muon Pt",1),("tPt", "Tau Pt",1)]
