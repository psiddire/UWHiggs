...
f = ROOT.TFile("results/MCLHE/AnalyzeEMu/"+name)
Interval = False
...
for di in Kinematics.names:
    ...
    scale_unc = []
    acc_unc = []
    nom_histo = f.Get(di+'/lhe0/e_m_VisibleMass')
    nom_int = nom_histo.Integral()
    nom_gen_histo = f.Get(di+'/lhe0/e_m_GenVisibleMass')
    nom_gen_int = nom_gen_histo.Integral()
    acc = nom_int/nom_gen_int
    for i in range(1, 9):
        var_histo = f.Get(di+'/lhe'+str(i)+'/e_m_VisibleMass')
        var_int = var_histo.Integral()
        var_gen_histo = f.Get(di+'/lhe'+str(i)+'/e_m_GenVisibleMass')
    ...
    pdf_int = []
    acc_unc = []
    nom_histo = f.Get(di+'/lhe9/e_m_VisibleMass')
    nom_int = nom_histo.Integral()
    nom_gen_histo = f.Get(di+'/lhe9/e_m_GenVisibleMass')
    nom_gen_int = nom_gen_histo.Integral()
    acc = nom_int/nom_gen_int
