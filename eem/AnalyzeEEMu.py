...
class AnalyzeEEMu(MegaBase):
  ...
  def kinematics(self, row):
    if row['e1Pt'] < 36 or abs(row['e1Eta']) >= 2.5:
      return False
  ...
  def obj1_id(self, row):
    return (bool(row['e1MVANoisoWP90']) and bool(row['e1PVDZ'] < 0.2) and bool(row['e1PVDXY'] < 0.045) and bool(row['e1PassesConversionVeto']) and bool(row['e1MissingHits'] < 2))#80
  ...
  def obj2_id(self, row):
    return (bool(row['e2MVANoisoWP90']) and bool(row['e2PVDZ'] < 0.2) and bool(row['e2PVDXY'] < 0.045) and bool(row['e2PassesConversionVeto']) and bool(row['e2MissingHits'] < 2))#80
  ...
  def copyrow(self, row):
    ...
    subtemp["e1PVDZ"] = row.e1PVDZ
    subtemp["e1PVDXY"] = row.e1PVDXY
    subtemp["e1PassesConversionVeto"] = row.e1PassesConversionVeto
    subtemp["e1MissingHits"] = row.e1MissingHits
    subtemp["e2PVDZ"] = row.e2PVDZ
    subtemp["e2PVDXY"] = row.e2PVDXY
    subtemp["e2PassesConversionVeto"] = row.e2PassesConversionVeto
    subtemp["e2MissingHits"] = row.e2MissingHits
    subtemp["mPt"] = row.mPt
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      if not self.is_data:
        ...
        e1id = wmc.function("e_id90_kit_ratio").getVal()#80
        ...
        e2id = wmc.function("e_id90_kit_ratio").getVal()#80
        zvtx = 0.991
        weight = newrow['GenWeight']*pucorrector(newrow['nTruePU'])*zvtx*e1iso*e1id*e2iso*e2id*trg
      ...
      if self.obj2_iso(newrow) and self.obj1_iso(newrow):
        ...
        if visibleMass(newrow) < 60 or visibleMass(newrow) > 120:
          continue
        ...
        m3Iso = 1
        if not self.is_data:
          m3ID = self.muonTightID(newrow['mPt'], abs(newrow['mEta']))
          m3Iso = self.muonLooseIsoTightID(newrow['mPt'], abs(newrow['mEta']))
        ...
        if not self.is_data:
          m3ID = self.muonTightID(newrow['mPt'], abs(newrow['mEta']))
          m3Iso = self.muonTightIsoTightID(newrow['mPt'], abs(newrow['mEta']))
