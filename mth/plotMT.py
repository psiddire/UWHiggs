...
mc_samples = [
    #'DYJetsToLL_M-50_TuneCP5_13TeV-amcatnloFXFX*',
...
for x in mc_samples:
    print x
    files.extend(glob.glob('results/%s/AnalyzeMuTauXG/%s' % (jobid, x)))
...
outputdir = 'plots/%s/AnalyzeMuTauXG/Dec2/%s/' % (jobid, s1)
...
#new_mc_samples.extend(['SMH', 'DYlow', 'EWKDiboson', 'ST', 'TT', 'DY', 'embed', 'QCD'])
