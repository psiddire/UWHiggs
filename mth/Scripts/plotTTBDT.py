...
for x in mc_samples:
    print x
    files.extend(glob.glob('../results/%s/AnalyzeMuTauTTbarBDT/%s' % (jobid, x)))
...
for j in jet:
    ...
    outputdir = '../plots/%s/AnalyzeMuTauTT/2016SelectionsBDT/%s/' % (jobid, s1)
    ...
    new_mc_samples = []
    new_mc_samples.extend(['QCD', 'DY', 'TT', 'ST', 'Diboson', 'EWK', 'DYlow', 'SMH'])
    ...
    for fn in foldername:
        ...
        for n,h in enumerate(histoname):
            ...
            plotter.plot_mc_vs_data(fn, ['VBF_LFV_HToMuTau_M125*', 'GluGlu_LFV_HToMuTau_M125*'], h[0], 1, xaxis = h[1], leftside=False, xrange=None, preprocess=None, show_ratio=True, ratio_range=1.5, sort=True, blind_region=True, control=s1, jets=j, year='2016', channel='h')
            plotter.save(h[0])
