...
for x in mc_samples:
    print x
    files.extend(glob.glob('../results/%s/AnalyzeMuTauSys/%s' % (jobid, x)))
...
for j in jet:
    ...
    outputdir = 'plots/%s/AnalyzeMuTauSys/2017SelectionsEmbed/%s/' % (jobid, s1)
    ...
    histoname = [("m_t_CollinearMass", "M_{col}(#mu, #tau) (GeV)", 1)]
