...
for j in Lists.jet:
    ...
    outputdir = 'plots/%s/AnalyzeMuTauSys/2018SelectionsEmbed/%s/' % (jobid, s[0])
    ...
    for h in Lists.colhisto:
        plotter.plot_mc_vs_data('', ['VBF_LFV_HToMuTau_M125*', 'GluGlu_LFV_HToMuTau_M125*'], h[0], 10, xaxis = h[1], leftside=False, xrange=None, preprocess=None, show_ratio=True, ratio_range=1.5, sort=True, blind_region=True, control=s1, jets=j, year='2018', channel='mutauh')
        plotter.save(h[0])
