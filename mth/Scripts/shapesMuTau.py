...
f = ROOT.TFile( 'shapes/shapesMuTau.root', 'RECREATE')
...
for di in dirs:
    ...
    f1.add(mttfakesup, -1)
    f1 = positivize(f1)
    f1 = f1.Rebin(len(binning)-1, "Fakes_CMS_TauFakeRate_13TeVUp", binning)
    f2 = tfakesdown.Clone()
    f2.add(mfakes)
    f2.add(mttfakesdown, -1)
    f2 = positivize(f2)
    f2 = f2.Rebin(len(binning)-1, "Fakes_CMS_TauFakeRate_13TeVDown", binning)
    f3 = tfakes.Clone()
    f3.add(mfakesup)
    f3.add(mtmfakesup, -1)
    f3 = positivize(f3)
