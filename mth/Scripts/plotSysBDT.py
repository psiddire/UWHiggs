...
for j in Lists.jet:
    ...
    outputdir = 'plots/%s/AnalyzeMuTauSys/2016SelectionsEmbedBDT/%s/' % (jobid, s[0])
    ...
    for h in Lists.bdthisto:
        plotter.plot_mc_vs_data('', ['VBF_LFV_HToMuTau_M125*', 'GluGlu_LFV_HToMuTau_M125*'], h[0], 10, xaxis = h[1], leftside=False, xrange=None, preprocess=None, show_ratio=True, ratio_range=1.5, sort=True, blind_region=True, control=s1, jets=j, year='2016', channel='mutauh')
        plotter.save(h[0])
