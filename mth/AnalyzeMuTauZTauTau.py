...
class AnalyzeMuTauZTauTau(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    super(AnalyzeMuTauZTauTau, self).__init__(tree, outfile, **kwargs)
    ...
    for row in self.tree:
      ...
      if self.is_embed:
        ...
        if newrow['tDecayMode'] == 0:
          ...
          dm = pow(0.975, 3)
        m_trg_sf = ws.function("m_trg24_27_embed_kit_ratio").getVal()
        m_id_sf = ws.function("m_id_embed_kit_ratio").getVal()
        m_iso_sf = ws.function("m_iso_binned_embed_kit_ratio").getVal()
        weight = weight*newrow['GenWeight']*tID*m_id_sf*m_iso_sf*dm
