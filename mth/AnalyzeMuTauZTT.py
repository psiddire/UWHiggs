...
class AnalyzeMuTauZTT(MegaBase):
  ...
  def copyrow(self, row):
    ...
    subtemp["m_t_PZeta"] = row.m_t_PZeta
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      if not self.is_data and not self.is_embed:
        ...
        mTrk = self.muTracking(newrow['mEta'])[0]
        ...
        if newrow['tZTTGenMatching']==5:
          ...
          tID = 1.0
        weight = newrow['GenWeight']*pucorrector(newrow['nTruePU'])*tEff*mID*tID*mTrk
        if self.is_DY:
