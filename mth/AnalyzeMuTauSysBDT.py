...
class AnalyzeMuTauSysBDT(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    self.xml_name = os.path.join(os.getcwd(), "bdtdata/datasetNoMFakes/weights/TMVAClassification_BDT.weights.xml")
  ...
  def var_d(self, row):
    return {'mPt' : row['mPt'], 'tPt' : row['tPt'], 'dPhiMuTau' : deltaPhi(row['mPhi'], row['tPhi']), 'dEtaMuTau' : abs(row['mEta'] - row['tEta']), 'type1_pfMetEt' : row['type1_pfMetEt'], 'm_t_collinearMass' : collMass(row), 'MTTauMET' : transverseMass(row['tPt'], row['tEta'], row['tPhi'], row['tMass'], row['type1_pfMetEt'], row['type1_pfMetPhi']), 'dPhiTauMET' : deltaPhi(row['tPhi'], row['type1_pfMetPhi'])}
    ...
    if self.is_mc:
      ...
      if ftesrow['tZTTGenMatching']==1 or ftesrow['tZTTGenMatching']==3:
        ...
        if ftesrow['tDecayMode'] == 0 or ftesrow['tDecayMode'] == 1:
          ...
          MVA = self.functor(**self.var_d(ftesrow))
      ...
      if bool(not self.is_DY and not self.is_DYlow):
        ...
        MVA = self.functor(**self.var_d(scalerow))
