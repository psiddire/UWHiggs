...
class AnalyzeMuTauFitBDTZMuMu(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    super(AnalyzeMuTauFitBDTZMuMu, self).__init__(tree, outfile, **kwargs)
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      if abs(visibleMass(newrow) - 91.2) > 5:
        continue

      if transverseMass(newrow['mPt'], newrow['mEta'], newrow['mPhi'], newrow['mMass'], newrow['type1_pfMetEt'], newrow['type1_pfMetPhi']) > 40:
        continue

      if newrow['type1_pfMetEt'] > 25:
        continue
      ...
      MVA = self.functor(**self.var_d_0)
      self.var_d_01 = {'mPt' : newrow['mPt'], 'tPt' : newrow['tPt'], 'dPhiMuTau' : deltaPhi(newrow['mPhi'], newrow['tPhi']), 'dEtaMuTau' : abs(newrow['mEta'] - newrow['tEta']), 'type1_pfMetEt' : newrow['type1_pfMetEt'], 'm_t_collinearMass' : collMass(newrow), 'MTTau\
MET' : transverseMass(newrow['tPt'], newrow['tEta'], newrow['tPhi'], newrow['tMass'], newrow['type1_pfMetEt'], newrow['type1_pfMetPhi']), 'dPhiTauMET' : deltaPhi(newrow['tPhi'], newrow['type1_pfMetPhi'])}
      MVA01 = self.functor01(**self.var_d_01)
      self.var_d_2 = {'mPt' : newrow['mPt'], 'tPt' : newrow['tPt'], 'dPhiMuTau' : deltaPhi(newrow['mPhi'], newrow['tPhi']), 'dEtaMuTau' : abs(newrow['mEta'] - newrow['tEta']), 'type1_pfMetEt' : newrow['type1_pfMetEt'], 'm_t_collinearMass' : collMass(newrow), 'MTTauM\
ET' : transverseMass(newrow['tPt'], newrow['tEta'], newrow['tPhi'], newrow['tMass'], newrow['type1_pfMetEt'], newrow['type1_pfMetPhi']), 'dPhiTauMET' : deltaPhi(newrow['tPhi'], newrow['type1_pfMetPhi']), 'vbfMass' : newrow['vbfMass'], 'vbfDeta' : newrow['vbfDeta']}
