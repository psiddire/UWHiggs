...
class AnalyzeMuTauXG(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    super(AnalyzeMuTauXG, self).__init__(tree, outfile, **kwargs)
  ...
  def begin(self):
    ...
    for x in range(0, namesize):
      self.book(names[x], "bdtDiscriminator", "XG Discriminator", 10, 0.0, 1.0)
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      bst = xgb.Booster(model_file='xgboost/xgb.model')
      data = np.array([newrow['mPt'], newrow['tPt'], deltaPhi(newrow['mPhi'], newrow['tPhi']), abs(newrow['mEta'] - newrow['tEta']), newrow['type1_pfMetEt'], collMass(newrow), transverseMass(newrow['tPt'], newrow['tEta'], newrow['tPhi'], newrow['tMass'], newrow['type1_pfMetEt'], newrow['type1_pfMetPhi']), deltaPhi(newrow['tPhi'], newrow['type1_pfMetPhi'])])
