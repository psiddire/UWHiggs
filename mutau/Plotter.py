...
class Plotter(object):
    def __init__(self, files, lumifiles, outputdir, blinder=None, forceLumi=-1):
        ...
        self.canvas.cd()
        self.pad = plotting.Pad(0., 0., 1., 1.) #full-size pad
        self.pad.Draw()
        self.pad.cd()
        self.lower_pad = None
        if blinder:
...
#            file_to_map = file_to_map[0]
    ...
    def add_ratio_plot(self, data_hist, mc_stack, x_range=None, ratio_range=0.5):
    ...
    def plot_mc_vs_data(self, folder, signal, variable, rebin=1, xaxis='',
                        ...
                        show_ratio=False, ratio_range=0.5, sort=False):
        ...
        mc_stack.GetHistogram().SetTitle("")
        if xrange:
        ...
        for sig in signal:
            ...
            signal_view = views.SubdirectoryView(signal_view, 'TightSS')
        ...
        # Draw data
        data_view = views.SubdirectoryView(self.get_view('data'), 'TightSS')
        if preprocess:
        ...
        if show_ratio:
            self.add_ratio_plot(data, mc_stack, xrange, ratio_range=0.5)
