...
class AnalyzeMuTauBDT(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    self.branches="mPt/F:tPt/F:dPhiMuTau/F:dEtaMuTau/F:puppiMetEt/F:m_t_collinearMass/F:MTTauMET/F:dPhiTauMET/F:weight/F"
    self.holders = []
  ...
  def filltree(self, row, weight):
    for varname, holder in self.holders:
      ...
      elif varname=="puppiMetEt":
        holder[0] = row['puppiMetEt']
      elif varname=="m_t_collinearMass":
        holder[0] = collMass(row)
      elif varname=="MTTauMET":
        holder[0] = transverseMass(row['tPt'], row['tEta'], row['tPhi'], row['tMass'], row['puppiMetEt'], row['puppiMetPhi'])
      elif varname=="dPhiTauMET":
        holder[0] = deltaPhi(row['tPhi'], row['puppiMetPhi'])
      elif varname=="weight": 
        holder[0] = weight
    self.tree1.Fill()
