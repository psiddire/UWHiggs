...
for x in mc_samples:
    print x
    files.extend(glob.glob('results/%s/AnalyzeMuTauFitNN/%s' % (jobid, x)))
...
for j in jet:
    ...
    outputdir = 'plots/%s/AnalyzeMuTauFitNN/Mar3MCFakes/%s/' % (jobid, s1)
    ...
    histoname = [("sigDiscriminator", "Signal Discriminator", 1)] 
