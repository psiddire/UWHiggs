...
class AnalyzeMuTauMC(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    super(AnalyzeMuTauMC, self).__init__(tree, outfile, **kwargs)
  ...
  def copyrow(self, row):
    ...
    subtemp["tPtold"] = row.tPt
    subtemp["type1_pfMetEtold"] = row.type1_pfMetEt
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      #  if nrow['tZTTGenMatching'] > 2 and nrow['tZTTGenMatching'] < 6:
      #    continue

      #print "Veto: ", nrow["tauVetoPt20Loose3HitsVtx"], nrow["tauVetoPt20TightMVALTVtx"]
      #print "Pair: ", nrow["mRelPFIsoDBDefaultR04"], nrow["mPt"], nrow["tByIsolationMVArun2v1DBoldDMwLTraw"], nrow["tPt"]
      ...
      if self.is_embed:
        ...
        ws.var("gt_pt").setVal(newrow['tPtold'])
        ...
        ws.var("gt2_pt").setVal(newrow['tPtold'])
