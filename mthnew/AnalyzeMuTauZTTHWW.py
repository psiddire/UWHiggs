...
class AnalyzeMuTauZTTHWW(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    self.frMuonWeight = self.mcWeight.frMuonWeight
    ...
    super(AnalyzeMuTauZTTHWW, self).__init__(tree, outfile, **kwargs)
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      if not self.obj1_tight(row) and self.obj1_loose(row) and self.obj2_tight(row):
        frMuon = self.frMuonWeight[self.absEta(abs(myMuon.Eta()))]
        mIso = 1
        if self.is_mc:
