...
class AnalyzeEMuBDT(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    self.branches="mPt/F:ePt/F:e_m_collinearMass/F:e_m_visibleMass/F:dPhiMuMET/F:dPhiEMET/F:dPhiMuE/F:MTMuMET/F:weight/F"
    self.holders = []
    ...
    for row in self.tree:
      ...
      if self.obj1_iso(row) and self.obj2_iso(row):
        if self.oppositesign(row):
          self.filltree(myEle, myMET, myMuon, weight)
