...
def PromoteDemoteSyst(h_btag_eff_b, h_btag_eff_c, h_btag_eff_oth, nbtag, bpt_1, bflavor_1, beta_1):
    ...
    SF = GetSF(1, bpt_1, bflavor_1, 0)
    if ( SF < 1 and myrand < (1-SF)):
        nbtag = nbtag-1
    if ( SF > 1 and myrand < ((1-SF)/(1-1.0/beff))):
        nbtag = nbtag+1

    SFup = GetSF(1, bpt_1, bflavor_1, 1)
    if ( SFup < 1 and myrand < (1-SFup)):
        nbtagup = nbtagup-1
    if ( SFup > 1 and myrand < ((1-SFup)/(1-1.0/beff))):
        nbtagup = nbtagup+1

    SFdown = GetSF(1, bpt_1, bflavor_1, -1)
    if ( SFdown < 1 and myrand < (1-SFdown)):
        nbtagdown = nbtagdown-1
    if ( SFdown > 1 and myrand < ((1-SFdown)/(1-1.0/beff))):
        nbtagdown = nbtagdown+1

    return [nbtag, nbtagup, nbtagdown]
