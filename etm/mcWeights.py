class mcWeights:
    ...
    def lumiWeight(self, weight):
        if self.is_embedB:
            weight = weight*(1.0/0.891)
        if self.is_embedC:
            weight = weight*(1.0/0.910)
        if self.is_embedD:
            weight = weight*(1.0/0.953)
        if self.is_embedE:
            weight = weight*(1.0/0.947)
        if self.is_embedF:
            weight = weight*(1.0/0.942)
        if self.is_embedG:
            weight = weight*(1.0/0.906)
        if self.is_embedH:
            weight = weight*(1.0/0.950)
        if self.is_DY:
        ...
        if self.is_ZZ:
            weight = weight*0.239*(16.91/12.14)
        ...
        if self.is_EWKZToNuNu:
            weight = weight*0.121
        if self.is_ZHTT:
            weight = weight*0.00512
        if self.is_Wminus:
            weight = weight*0.00514
        if self.is_Wplus:
            weight = weight*0.00516
        if self.is_STtantitop:
            weight = weight*0.07969
        if self.is_STttop:
            weight = weight*0.07969
