...
for x in mc_samples:
    print x
    files.extend(glob.glob('../results/%s/AnalyzeEMuSys/%s' % (jobid, x)))
...
for j in jet:
    ...
    outputdir = 'plots/%s/AnalyzeEMuSys/2016SelectionsEmbed/%s/' % (jobid, s1)
    ...
    histoname = [("e_m_CollinearMass", "Electron + Muon Collinear Mass (GeV)", 1)]
