...
for j in Lists.jetc:
    ...
    for fn in Lists.foldername:
        ...
        for n,h in enumerate(Lists.histoname):
            plotter.plot_mc_vs_data(fn, ['VBF_LFV_HToETau_M125*', 'GluGlu_LFV_HToETau_M125*'], h[0], 1, xaxis = h[1], leftside=False, xrange=None, preprocess=None, show_ratio=True, ratio_range=1.5, sort=True, blind_region=True, control=s1, jets=j, year='2016', channel='etaumu')
            plotter.save(h[0])
