...
for x in mc_samples:
    print x
    files.extend(glob.glob('results/%s/AnalyzeEETDeep/%s' % (jobid, x)))
...
for d in dirs:
    ...
    outputdir = 'plots/%s/AnalyzeEETDeep/%s/' % (jobid, d)
    ...
    for fn in foldername:
        ...
        for n,h in enumerate(histoname):
            plotter.plot_mc_vs_data(fn, [], h[0], 1, xaxis = h[1], leftside=False, xrange=None, preprocess=None, show_ratio=True, ratio_range=1.5, sort=True, blind_region=True, control=d, jets='', year='2016', channel='eetauh')
            plotter.save(h[0])
